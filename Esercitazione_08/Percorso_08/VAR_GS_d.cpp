...
int main (int argc, char *argv[]){
   ...
   //Calcolo il primissimo valore dell'energia, con mu = 1.0 e sigma = 0.5. E' l'esercizio 08.1.
   ...
   for(item=0; item<annealing; item++){ //Cicli della temperatura che decresce.
      Move_parameters(temp);
      temp -= delta_t; //Abbasso la temperatura per il prossimo ciclo di un valore delta_t. 
   }

   cout << "Gli ultimi valori selezionati dal problema sono: energia, incertezza, mu, sigma: " << endl;
   cout << en_old << "  " << incer_old << "  " << mu_old << "  " << sigma_old << endl; 
